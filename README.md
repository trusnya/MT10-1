...
Когда вы опишете объекты веб-хранилища, попробуйте сами создать парочку! С помощью метода `setItem(key, value)` нужно добавить в `sessionStorage` и в `localStorage` по одному объекту. Для этого в DevTools необходимо открыть вкладку "Console", после чего ввести соответствующую команду. В тот же файл `exercise2.md` необходимо будет прикрепить 4 скриншота:  
